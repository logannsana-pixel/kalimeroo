...
import { Store, ArrowLeft, Mail, Loader2 } from "lucide-react";
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (roleData?.role !== "restaurant_owner") {
        toast.error("Compte non restaurant");
  ...
  const handleSignup = async (restaurantData: any) => {
    ...
    try {
      ...
      if (logoUrl && data.user) {
        setTimeout(() => supabase.from('restaurants').update({ image_url: logoUrl }).eq('owner_id', data.user!.id), 1000);
      }
