...
const statusConfig: Record<OrderStatus, { label: string; color: string; icon: React.ReactNode }> = {
  pending: { label: "En attente", color: "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400", icon: <Clock className="w-3 h-3" /> },
  accepted: { label: "Acceptée", color: "bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400", icon: <CheckCircle className="w-3 h-3" /> },
  confirmed: { label: "Confirmée", color: "bg-blue-100 text-blue-800", icon: <CheckCircle className="w-3 h-3" /> },
  preparing: { label: "En préparation", color: "bg-primary/10 text-primary", icon: <Package className="w-3 h-3" /> },
  ready: { label: "Prête", color: "bg-accent/10 text-accent", icon: <CheckCircle className="w-3 h-3" /> },
  pickup_pending: { label: "Attente livreur", color: "bg-purple-100 text-purple-800", icon: <Truck className="w-3 h-3" /> },
  pickup_accepted: { label: "Livreur en route", color: "bg-indigo-100 text-indigo-800", icon: <Truck className="w-3 h-3" /> },
  picked_up: { label: "Récupérée", color: "bg-cyan-100 text-cyan-800", icon: <Truck className="w-3 h-3" /> },
  delivering: { label: "En livraison", color: "bg-primary/15 text-primary", icon: <Truck className="w-3 h-3" /> },
  delivered: { label: "Livrée", color: "bg-accent/10 text-accent", icon: <CheckCircle className="w-3 h-3" /> },
  cancelled: { label: "Annulée", color: "bg-destructive/10 text-destructive", icon: <XCircle className="w-3 h-3" /> },
};
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `user_id=eq.${user.id}` }, (payload: any) => {
          ...
          if (payload.new?.status && payload.old?.status !== payload.new.status) {
            ...
            if (status) toast.info(`Commande ${status.label.toLowerCase()}`);
          }
        })
        .subscribe();
    ...
    <div className="min-h-screen flex flex-col pb-20 md:pb-0 bg-background">
      ...
      <div className="flex px-4 pt-3 gap-2">
        <button
          ...
          className={cn("flex-1 py-2 rounded-full text-xs font-medium transition-all",
        ...
        {displayOrders.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {activeTab === 'active' ? "Vos commandes actives apparaîtront ici" : "Vos commandes passées apparaîtront ici"}
            </p>
            {activeTab === 'active' && (
              <Button onClick={() => navigate("/home")} className="rounded-full text-sm">
          ...
          <div className="space-y-3">
            {displayOrders.map((order) => {
              ...
              return (
                <div key={order.id} className={cn("bg-card rounded-2xl overflow-hidden shadow-sm", isActive && "ring-1 ring-primary/20")}>
                  {/* Order Header */}
                  <button className="w-full p-4 text-left" onClick={() => setExpandedOrder(isExpanded ? null : order.id)}>
                    <div className="flex items-start gap-3">
                      <div className="w-12 h-12 rounded-xl overflow-hidden flex-shrink-0">
                        <img src={order.restaurants.image_url || "/placeholder.svg"} alt="" className="w-full h-full object-cover" />
                      </div>
                    ...
                    <div className="px-4 pb-4 space-y-3 border-t border-border/30 pt-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {order.restaurants.owner_id && (
                          <ChatInterface orderId={order.id} receiverId={order.restaurants.owner_id} receiverName={order.restaurants.name} />
                        )}
