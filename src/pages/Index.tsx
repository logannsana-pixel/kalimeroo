...
const FILTER_CHIPS = [
  ...
  { label: "⭐ Top notés", value: "top_rated" },
  { label: "🚀 Rapide", value: "fast" },
  { label: "💰 Gratuit", value: "free_delivery" },
...
const Index = () => {
  ...
  const filteredRestaurants = useMemo(() => {
    ...
    // Quick filter
    if (activeFilter === "top_rated") filtered = filtered.filter(r => (r.rating || 0) >= 4);
    if (activeFilter === "fast") filtered = filtered.filter(r => parseInt(r.delivery_time || "99") <= 30);
    if (activeFilter === "free_delivery") filtered = filtered.filter(r => r.delivery_fee === 0);
    // Advanced filters
    ...
    if (advancedFilters.openNow) filtered = filtered.filter(r => r.isOpen !== false);
    if (advancedFilters.cuisineTypes.length > 0) filtered = filtered.filter(r => advancedFilters.cuisineTypes.includes(r.cuisine_type || ""));
    // Sort
    if (advancedFilters.sortBy === "rating") filtered = [...filtered].sort((a, b) => (b.rating || 0) - (a.rating || 0));
    if (advancedFilters.sortBy === "delivery_time") filtered = [...filtered].sort((a, b) => parseInt(a.delivery_time || "99") - parseInt(b.delivery_time || "99"));
    if (advancedFilters.sortBy === "delivery_fee") filtered = [...filtered].sort((a, b) => (a.delivery_fee || 0) - (b.delivery_fee || 0));
    return filtered;
  }, [restaurants, activeFilter, advancedFilters]);
  ...
  return (
    <PageTransition>
      ...
      <main className="min-h-screen bg-background pb-24">
        ...
        <section className="px-4 mt-3"><MarketingBanner /></section>

        {/* Loyalty Widget */}
        {isConnected && (
          <section className="px-4 mt-3"><LoyaltyWidget /></section>
        )}

        {isConnected && <section className="mt-4"><FavoritesSection /></section>}
        {isConnected && hasAddress && <section className="px-4 mt-4"><OrderAgainSection /></section>}
        <div className="mt-4"><FoodCategories /></div>
            ...
            {FILTER_CHIPS.map(chip => (
              <button key={chip.value} onClick={() => setActiveFilter(chip.value)}
                className={`px-3 py-1.5 rounded-full text-xs font-body font-medium whitespace-nowrap transition-all ${
                  activeFilter === chip.value ? "bg-primary text-primary-foreground" : "bg-card border border-primary/20 text-primary"
                }`}>
                {chip.label}
              </button>
            ))}
        ...
        <div className="mt-4"><SponsoredRestaurants /></div>

        {/* Restaurants Grid */}
        <section className="px-4 mt-5 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-display font-bold text-foreground">Restaurants populaires</h2>
            <button onClick={() => navigate("/restaurants")} className="text-primary text-xs font-body font-medium flex items-center gap-1">
              Voir tout <ArrowRight className="w-3 h-3" />
            </button>
          </div>
          {loading ? (
            <div className="space-y-3">{[...Array(3)].map((_, i) => <Skeleton key={i} className="h-56 rounded-2xl" />)}</div>
            ...
            <Card className="border border-border rounded-2xl text-center py-8 bg-card">
              <CardContent className="p-0">
                ...
                <p className="text-sm font-display font-medium text-foreground mb-1">Aucun restaurant trouvé</p>
                <p className="text-xs text-muted-foreground font-body">Essayez un autre filtre</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-3">
              {filteredRestaurants.map((r, i) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className={`border-none rounded-2xl overflow-hidden bg-card shadow-card cursor-pointer transition-all hover:shadow-elevated hover:scale-[1.01] active:scale-[0.98] relative ${isClosed ? "grayscale opacity-60" : ""}`}
                      onClick={() => navigate(`/restaurant/${r.id}`)}
                    >
                      <div className="relative h-40">
                        <LazyImage src={r.image_url || "/placeholder.svg"} alt={r.name} className="w-full h-full object-cover" />
                        {hasAddress && (
                      ...
                      <CardContent className="p-3 space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-xs font-body pt-1">
                          <span className="flex items-center gap-1 font-medium text-foreground">
                            <Star className="w-3.5 h-3.5 fill-primary text-primary" />{r.rating?.toFixed(1) || "—"}
                          </span>
                          <span className="flex items-center gap-1 text-muted-foreground">
                            <Clock className="w-3.5 h-3.5" />{r.delivery_time || "20-30"} min
                          </span>
                          <span className="text-muted-foreground">
                            🛵 {r.delivery_fee ? `${r.delivery_fee.toLocaleString()} F` : "Gratuit"}
                          </span>
                        </div>
                      </CardContent>
                      {isClosed && (
