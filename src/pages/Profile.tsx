...
import { LogOut, Phone, MapPin, ChevronLeft, Moon, ChevronRight, Star } from "lucide-react";
...
export default function Profile() {
  useDocumentTitle("Mon profil | Kalimero");
  ...
  const [formData, setFormData] = useState({ full_name: '', phone: '', address: '', city: '', district: '', avatar_url: '' });

  useEffect(() => { fetchProfile(); fetchOrderCount(); fetchLoyalty(); }, [user]);
  ...
  const fetchLoyalty = async () => {
    ...
    if (data) { setLoyaltyPoints(data.points); setLoyaltyLevel(data.level); }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col pb-20 bg-background">
        ...
        <div className="px-4 space-y-4"><Skeleton className="h-40 rounded-2xl" /><Skeleton className="h-32 rounded-2xl" /></div>
        <BottomNav />
      </div>
    );
  }
      ...
      <div className="min-h-screen flex flex-col pb-20 md:pb-0 bg-background">
        ...
        <div className="bg-gradient-to-b from-primary/15 to-background relative">
          <div className="absolute top-4 left-4">
            <button onClick={() => navigate(-1)} className="w-8 h-8 flex items-center justify-center md:hidden"><ChevronLeft className="w-5 h-5" /></button>
          </div>
          <div className="absolute top-4 right-4">
            <button onClick={signOut} className="text-muted-foreground hover:text-foreground p-2"><LogOut className="h-4 w-4" /></button>
          </div>
          <div className="pt-12 pb-6 px-4 text-center">
            <div className="relative inline-block mb-3">
              <div className="w-20 h-20 rounded-full bg-muted overflow-hidden border-4 border-background shadow-md">
                {formData.avatar_url ? <img src={formData.avatar_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-2xl">👤</div>}
              </div>
            </div>
            ...
            <div className="flex justify-center gap-6 mt-3 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><Star className="w-3 h-3 fill-primary text-primary" /><strong className="text-foreground">{loyaltyPoints}</strong> Stars</span>
            </div>
          </div>
        </div>
        ...
        <main className="flex-1 px-4 max-w-lg mx-auto w-full space-y-4">
          ...
          {loyaltyPoints > 0 && (
            <div className="bg-card rounded-2xl p-4">
              <h3 className="text-sm font-semibold mb-2">Mes avantages</h3>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Star className="w-4 h-4 text-primary fill-primary" />
                <span><strong className="text-foreground">{loyaltyPoints}</strong> Kalimero Stars · Niveau <strong className="text-primary capitalize">{loyaltyLevel}</strong></span>
              </div>
            </div>
          )}
          ...
          <div className="bg-card rounded-2xl p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2"><Moon className="w-4 h-4 text-muted-foreground" /><span className="text-sm">Mode sombre</span></div>
          ...
          {userRole === 'restaurant_owner' && (
            <button onClick={() => navigate('/restaurant-dashboard')} className="w-full bg-card rounded-2xl p-4 flex items-center justify-between">
              <span className="text-sm font-medium">Dashboard Restaurant</span><ChevronRight className="w-4 h-4 text-muted-foreground" />
            </button>
          )}
          {userRole === 'delivery_driver' && (
            <button onClick={() => navigate('/delivery-dashboard')} className="w-full bg-card rounded-2xl p-4 flex items-center justify-between">
              <span className="text-sm font-medium">Dashboard Livreur</span><ChevronRight className="w-4 h-4 text-muted-foreground" />
            </button>
          )}
          ...
          <div className="bg-card rounded-2xl p-4 space-y-3">
            ...
            <button onClick={signOut} className="w-full py-2 text-sm text-destructive font-medium hover:bg-destructive/5 rounded-lg transition-colors">Déconnexion</button>
          </div>
        </main>
